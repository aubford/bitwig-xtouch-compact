...
host.defineSysexIdentityReply("F0 7E ?? 06 02 47 6B 00 19 00 ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? ?? F7");
host.defineMidiPorts(1, 1);

if (host.platformIsWindows())
	host.addDeviceNameBasedDiscoveryPair(["Akai MPK49"], ["Akai MPK49"]);
else if (host.platformIsMac())
	host.addDeviceNameBasedDiscoveryPair(["Akai MPK49 Port 1"], ["Akai MPK49 Port 1"]);
else if (host.platformIsLinux())
	host.addDeviceNameBasedDiscoveryPair(["Akai MPK49 MIDI 1"], ["Akai MPK49 MIDI 1"]);
...
//	printMidi(status, index, val);
	...
	if (buttonPressed)
	{
		...
		{
			case CC.PLAY:
				isShift ? transport.returnToArrangement() : isPlay ? transport.restart() : transport.play();
				break;
			case CC.STOP:
